...
fn cleanup_test_chains(chain_prefix: &str) {
    eprintln!("[debug] cleanup_test_chains() started");
    let filter = fortexa::modules::iptables::IptablesFilter::new(chain_prefix)
        .expect("Failed to create IptablesFilter for cleanup");
    filter.cleanup().expect("Failed to cleanup iptables test chains");
    eprintln!("[debug] cleanup_test_chains() completed");
}
