...
async fn test_rules_initialization() {
    ...
    assert!(
        rules.is_ok(),
        "Rules manager should initialize successfully"
    );
}
...
async fn test_rules_operations() {
    ...
    assert!(
        modified_rules.input.blocked_ips.contains(&test_ip),
    ...
    assert_eq!(
        modified_rules.input.blocked_ips.len(),
        expected_ip_count,
        "Blocked IPs count should be {} after insertion",
        expected_ip_count
    );
    ...
    assert!(
        modified_rules.input.blocked_ports.contains(&test_port),
        "Port should be in blocked list"
    );
    ...
    assert_eq!(
        modified_rules.input.blocked_ports.len(),
        expected_port_count,
        "Blocked ports count should be {} after insertion",
        expected_port_count
    );
    ...
    assert!(
        modified_rules.input.whitelisted_ips.contains(&whitelist_ip),
        "IP should be in whitelist"
    );
    ...
    modified_rules
        .input
        .whitelisted_ports
        .insert(whitelist_port);
    assert!(
        modified_rules
            .input
            .whitelisted_ports
            .contains(&whitelist_port),
        "Port should be in whitelist"
    );
    ...
    assert!(
        !modified_rules.input.blocked_ips.contains(&test_ip),
        "IP should be removed from blocked list"
    );

    modified_rules.input.blocked_ports.remove(&test_port);
    assert!(
        !modified_rules.input.blocked_ports.contains(&test_port),
        "Port should be removed from blocked list"
    );
    ...
    assert!(
        modified_rules.output.blocked_ips.contains(&output_ip),
        "IP should be in output blocked list"
    );

    // Test removing all rules from a category
    modified_rules.output.blocked_ips.clear();
    assert!(
        modified_rules.output.blocked_ips.is_empty(),
        "Output blocked IPs should be empty after clear"
    );
    assert_eq!(
        modified_rules.output.blocked_ips.len(),
        0,
        "Expected 0 IPs after clear, got {}",
        modified_rules.output.blocked_ips.len()
    );
}
